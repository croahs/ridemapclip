...
function encode(records: RecordMesg[]) {
  const encoder = new Encoder();
  encoder.onMesg(Profile.MesgNum.FILE_ID, { type: "activity", manufacturer: "development", timeCreated: start } as FileIdMesg);
...
export function rideFit(name: string, variant = 0, seconds = 300) {

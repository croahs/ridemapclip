...
| Topic | Decision |
|---|---|
| Git history | Purge `fitexample/` and `vibenotesridemapclip.txt` from all history (public repo). See Phase 0. |
...
## Phase 0 — history purge (in progress)

- Rewritten history exists (all four commits, trees identical apart from the
  two removed paths). Local `main` and the force-push to GitHub still need
  Peder's permission; the agent was blocked from doing either.
- After the force-push: Peder asks GitHub Support to purge cached views of the
  old commits (`c0a9dca`, `b547536`), since old SHAs stay reachable by URL
  until then. The repo had 0 forks at purge time.

## Phase 1 — hygiene and gates

- Switch to pnpm: `packageManager` + `engines` (Node ≥ 22.13) in
  `package.json`, `pnpm-lock.yaml`, allow build scripts only for packages that
  need them (esbuild). Update README commands (`npm.cmd` → `pnpm`).
- Add `.gitattributes` (`* text=auto eol=lf`) and renormalize once.
- Delete dead code:
  - Parked OAuth: `app/api/intervals/{connect,callback,disconnect,status}`,
    `lib/intervals-session.ts`, commented blocks in `app/page.tsx`, OAuth
    parts of README and `.env.example`. Tag the last commit that has it as
    `parked/oauth` before deleting.
  - `app/api/upload/route.ts` (legacy, unused by the UI); move its tests in
    `tests/fit.test.ts` to call `parseFit`/`validateFitBatch` directly.
  - `.github/copilot-instructions.md` (stale; Copilot reads AGENTS.md).
  - Unused Geist font loading in `app/layout.tsx` (body uses Arial) and unused
    CSS classes `selectedFile`, `stats`, `trackName`.
- Scripts: `typecheck` (`tsc --noEmit`), `check` = typecheck + lint + unit
  tests. GitHub Actions CI runs `pnpm check` on push and PR.
- Browser tests must run on any machine: replace the private `fitexample/`
  dependency with small synthetic FIT fixtures generated by the Garmin
  encoder (as `tests/fit.test.ts` already does), drop the exact 198/194 file
  counts, and use Playwright's bundled Chromium (Edge optional).

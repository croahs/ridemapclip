...
- **Colors:** a distinct colour per ride, or a gradient by start date or average speed (one colour per ride), or by elevation (green to red), 30-second power, speed or heart rate along the route, where the trail changes colour and the dot shows the current value. Point scales span the whole batch from the 2nd to the 98th percentile, so rides are comparable and one spike does not flatten the scale. A legend with the range is drawn on the map and in the video; rides without the data are grey and counted below the map.
